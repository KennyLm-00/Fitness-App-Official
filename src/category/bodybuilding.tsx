...
const Bodybuilding: React.FC = () => {
  ...
  const [BodyBuildingPosts, setBodyBuildingPosts] = useState<any[]>([]);
  const history = useHistory();

  useEffect(() => {
    const fetchBodyBuildingPosts = async () => {
      const BodyBuildingCollection = collection(firestore, 'posts');
      const BodyBuildingQuery = query(BodyBuildingCollection, where('category', '==', 'bodybuilding'));

      try {
        const querySnapshot = await getDocs(BodyBuildingQuery); // Use getDocs instead of get
        const BodyBuildingPostsData = querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
        console.log('BodyBuilding Posts Data:', BodyBuildingPostsData);
        setBodyBuildingPosts(BodyBuildingPostsData);
      } catch (error) {
        console.error('Error fetching BodyBuilding posts:', error);
      }
    };
  ...
  const handleLike = async (postId: string, userId: string) => {
    try {
      const likedPostIndex = BodyBuildingPosts.findIndex((post) => post.id === postId);

      if (likedPostIndex !== -1) {
        const likedBy = BodyBuildingPosts[likedPostIndex].likedBy;
        ...
        setBodyBuildingPosts((prevPosts) => {
