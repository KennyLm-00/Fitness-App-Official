...
const Powerlifting: React.FC = () => {
  ...
  const [powerliftingPosts, setPowerliftingPosts] = useState<any[]>([]);
  const history = useHistory();

  useEffect(() => {
    const fetchPowerliftingPosts = async () => {
      const powerliftingCollection = collection(firestore, 'posts');
      const powerliftingQuery = query(powerliftingCollection, where('category', '==', 'powerlifting'));

      try {
        const querySnapshot = await getDocs(powerliftingQuery); // Use getDocs instead of get
        const powerliftingPostsData = querySnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
        console.log('Powerlifting Posts Data:', powerliftingPostsData);
        setPowerliftingPosts(powerliftingPostsData);
      } catch (error) {
        console.error('Error fetching powerlifting posts:', error);
      }
    };
  ...
  const handleLike = async (postId: string, userId: string) => {
    try {
      const likedPostIndex = powerliftingPosts.findIndex((post) => post.id === postId);

      if (likedPostIndex !== -1) {
        const likedBy = powerliftingPosts[likedPostIndex].likedBy;
        ...
        setPowerliftingPosts((prevPosts) => {
